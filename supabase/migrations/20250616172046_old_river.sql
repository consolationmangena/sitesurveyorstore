...
-- Drop existing function and recreate to ensure consistency
...
-- Drop existing policies and recreate
DROP POLICY IF EXISTS "Anyone can insert downloads" ON app_downloads;
DROP POLICY IF EXISTS "Users can view own downloads" ON app_downloads;
...
-- Drop existing policies and recreate
DROP POLICY IF EXISTS "Users can manage own favorites" ON app_favorites;
...
DROP POLICY IF EXISTS "Users can view own blog views" ON blog_views;
...
-- Drop existing policies and recreate
DROP POLICY IF EXISTS "Users can manage own blog likes" ON blog_likes;
...
-- Drop existing policies and recreate
DROP POLICY IF EXISTS "Anyone can create requests" ON solution_requests;
DROP POLICY IF EXISTS "Users can view own requests" ON solution_requests;
...
-- Drop existing trigger and recreate
DROP TRIGGER IF EXISTS update_solution_requests_updated_at ON solution_requests;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM applications WHERE name = 'QGIS') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM applications WHERE name = 'SurveyPro') THEN
        INSERT INTO applications (
            name, description, category_id, app_type, price, 
            author_name, license, homepage_url,
            tags, features, download_count, is_featured, is_active
        ) VALUES (
            'SurveyPro',
    ...
    IF NOT EXISTS (SELECT 1 FROM applications WHERE name = 'FieldMapper') THEN
        INSERT INTO applications (
            name, description, category_id, app_type, price, 
            author_name, license, homepage_url, repo_url,
            tags, features, download_count, is_featured, is_active
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM blog_posts WHERE slug = 'getting-started-open-source-gis') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM blog_posts WHERE slug = 'modern-surveying-techniques') THEN
        INSERT INTO blog_posts (
            title, slug, excerpt, content, category, tags,
            author_name, author_role, read_time, is_featured, is_published, published_at
        ) VALUES (
